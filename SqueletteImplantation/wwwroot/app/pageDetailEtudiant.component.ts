...
export class PageDetailEtudiantComponent  { 
    ...
    validationSiEtudiantExisteEtAjoutBD(noDa: number) {
        ...
        this.http .post("api/EtudiantConnecte", JSON.stringify({noDa: noDa}), { headers: headers }).subscribe(r=> {
            if(r.status == 200) {
    ...
    validationChampSaisi(): boolean {
        ...
        if (this.etudiant.noDa.toString().length == 0 || this.etudiant.nom === "" || this.etudiant.prenom === "" ||
